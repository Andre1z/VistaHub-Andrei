...
try {
    ...
    if (empty($id_empresa) || empty($nombre) || empty($apellidos) || empty($estado)) {
    ...
    $sql = "INSERT INTO empleados (id_empresa, nombre, apellidos, dni, telefono, email, puesto, departamento, salario, fecha_contratacion, estado, direccion, ciudad, codigo_postal) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    if (!$stmt->execute()) {
        throw new Exception('Error al agregar empleado: ' . $stmt->error);
    }
